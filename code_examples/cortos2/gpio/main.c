...
void main () 
{
	...
	write_to_gpio_config_register (0xff);//11110000 1-output, 0-input (default should have all ins)
	...
	while(count<10){
		//uint32_t inval = read_from_gpio_din_register ();
		write_to_gpio_dout_register(count);
	         cortos_printf("GPIO_IN = 0x%x\tGPIO_OUT = 0x%x\n", read_from_gpio_din_register(), read_from_gpio_dout_register());
	 	count++;
